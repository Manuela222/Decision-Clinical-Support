...
ASSUMPTION FLAGGED FOR FEASIBILITY REPORT:
Quan's algorithm defines two separate categories, "Hypertension,
uncomplicated" (401.x) and "Hypertension, complicated" (402-405.x, the
codes shared with CHF/renal failure). Per the original Elixhauser (1998)
formulation and common practice, this implementation merges both into a
single "Hypertension" comorbidity group, since the project spec treats
hypertension as one condition/comorbidity, not two. Codes shared between
Hypertension-complicated and Congestive Heart Failure / Renal Failure
(e.g. 402.x1, 404.x, 403.x) are counted toward BOTH groups if present,
consistent with the original algorithm's design (a patient can have both
conditions).
...
This mapping was reconstructed from the published literature rather than
copied from a validated code package, and has NOT been unit-tested against
a reference implementation (e.g. AHRQ's SAS/R software) row-for-row. It is
adequate for a feasibility go/no-go check but MUST be revalidated (ideally
by diffing against the `comorbidity` R package or AHRQ's software output on
the same MIMIC-III extract) before being used for anything beyond Phase 0.
"""

import re
from typing import Dict, List, Set

# Each entry: category name -> list of code-matching rules.
# A rule is either an exact prefix string (matched via startswith) or a
# (low, high) numeric range tuple applied to the first 3 digits (as int),
# with an optional set of allowed 4th/5th-digit suffixes ignored (we match
# on prefix, which is the standard simplification for administrative data).
...
ELIXHAUSER_ICD9_PREFIXES: Dict[str, List[str]] = {
    ...
    "Solid tumor without metastasis": [
        "14", "15", "16", "17",  # 140-172 (approx, refined below)
        "18", "19",  # 174-195 (approx, refined below)
    ],
...
# 174-195) rather than the crude single/double-digit prefixes above, which
# would over-match. Replace with an explicit 3-digit range check instead.
_SOLID_TUMOR_RANGES = [(140, 172), (174, 195)]

del ELIXHAUSER_ICD9_PREFIXES["Solid tumor without metastasis"]
